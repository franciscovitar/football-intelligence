...
| RSSSF Argentina 2016 | The reviewed `arg2016.html` document explicitly permits copying the document in whole or part with proper acknowledgement to author Osvaldo José Gorgazzi; this is treated as document-specific permission, not a blanket RSSSF licence | Yes, bounded to the reviewed page | Yes for this reviewed document with required acknowledgement; no broader RSSSF redistribution claim | Direct public HTML GET plus strict competition-section parser; server-declared `windows-1252`; fail-closed 242-match structure gates | ARG_LPF short tournament 2016: 242/242 matches, 30 provider-local club labels, date/venue/result/phase; no complete regular-round player appearance/minutes layer and no fabricated kickoff time | Approved bounded historical fixture/result backbone; player evidence remains unsupported/missing |
...
- RSSSF Argentina 2016: bounded real-source spike run `33084890010` recovered
  242/242 tournament matches; raw HTML digest at acquisition
  `sha256:de9bea28bd098cd7094581cf4595db9d95496d42763db8530154e3ee8b0eb566`.
  Product-code runtime verification run `33095161343` then produced 1,694
  observations (1,210 match + 484 team-name), zero player observations, zero
  fabricated `kickoff_at` observations, and resolved 242/242 logical matches
  under canonical calendar-season `ARG_LPF:2016`. Raw HTML remains outside the
  repository; see `docs/audits/rsssf-arg-lpf-2016-match-backbone.md`.
