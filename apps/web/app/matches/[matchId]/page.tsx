...
function ratingClass(score: number): string {
  const tone = score >= 9 ? styles.world : score >= 8 ? styles.excellent : score >= 7 ? styles.good : score >= 6 ? styles.solid : score >= 5 ? styles.weak : styles.poor;
  return `${styles.rating} ${tone}`;
}

function Confidence({ value }: { value: number }) {
  return (
    <span className={styles.confidence} title={`Confianza ${value}/100`}>
...
function ChannelScores({ facts, expert, fan }: { facts: number | null; expert: number | null; fan: number | null }) {
  return (
    <div className={styles.channels} aria-label="Canales de evidencia">
...
function StatComparison({ label, home, away, suffix = "" }: { label: string; home: number | null; away: number | null; suffix?: string }) {
  ...
  return (
    <div className={styles.statRow}>
      ...
      <div>
        <span>{label}</span>
        <span className={styles.statTrack} aria-hidden="true"><i style={{ width: `${homeShare}%` }} /></span>
      </div>
...
function TeamVerdict({ review }: { review: MatchTeamReview }) {
  return (
    <article className={`panel ${styles.teamCard}`}>
      ...
      <div className={styles.confidenceRow}><span>Confianza de la conclusión</span><Confidence value={review.confidence} /></div>
...
function PlayerList({ teamId, teamName, players }: { teamId: string; teamName: string; players: MatchPlayerReview[] }) {
  ...
  return (
    <section className={`panel ${styles.playerPanel}`}>
      ...
      <div className={styles.playerHeader} aria-hidden="true"><span>Jugador</span><span>Facts</span><span>Exp.</span><span>Fans</span><span>FI</span><span>Conf.</span></div>
      <div className={styles.playerList}>
        {rows.map((player) => (
          <details className={styles.player} key={player.playerId}>
            <summary>
              <span className={styles.playerName}><strong>{player.playerName}</strong><small>{player.starter ? "Titular" : "Suplente"} · {player.minutes ?? "—"}&apos; · {player.roleLabel ?? "rol no especificado"}</small></span>
            ...
            <div className={styles.playerDetail}>
              ...
              {(player.positiveNotes.length > 0 || player.negativeNotes.length > 0) && <div className={`${styles.prosCons} ${styles.compact}`}>
...
export default async function MatchPage({ params }: { params: Promise<{ matchId: string }> }) {
  ...
  return (
    <main className={`page-shell ${styles.matchPage}`}>
      <section className={styles.scoreboard}>
        ...
        <p className={styles.scoreNote}>La nota FI evalúa la actuación, no sólo el resultado. Facts, expertos e hinchas son canales auditables; la síntesis no usa pesos fijos.</p>
      </section>

      <section className={`context-strip ${styles.context}`}>
      ...
      <section className={`dashboard-grid ${styles.topGrid}`}>
        <article className={`panel ${styles.factsPanel}`}>
          ...
          <div className={styles.statTeams}><strong>{match.home.shortName ?? match.home.name}</strong><strong>{match.away.shortName ?? match.away.name}</strong></div>
          ...
          <p className={styles.providerNote}>El xG mostrado conserva el modelo del proveedor; no promediamos modelos incompatibles.</p>
        </article>

        <article className={`panel ${styles.reading}`}>
          <div className="section-heading"><div><p className="eyebrow">LECTURA FI</p><h2>Qué cuenta el partido</h2></div></div>
          <p>{match.reading}</p>
          <div className={styles.takeaways}>{match.takeaways.map((item, index) => <div key={item}><span>{String(index + 1).padStart(2, "0")}</span><p>{item}</p></div>)}</div>
        </article>
      </section>

      {homeReview && awayReview && <section className={`dashboard-grid ${styles.teamGrid}`}><TeamVerdict review={homeReview} /><TeamVerdict review={awayReview} /></section>}

      <section className={`dashboard-grid ${styles.managerGrid}`}>
        {match.managerReviews.map((review) => (
          <article className={`panel ${styles.manager}`} key={review.managerId}>
            <div className={styles.cardHead}><div><p className="eyebrow">TÉCNICO · {review.teamName}</p><h2>{review.managerName}</h2></div><div className={ratingClass(review.finalScore)}><strong>{review.finalScore.toFixed(1)}</strong><small>FI</small></div></div>
            <ChannelScores facts={review.factsScore} expert={review.expertScore} fan={review.fanScore} />
            <div className={styles.confidenceRow}><span>Confianza</span><Confidence value={review.confidence} /></div>
            <p className={styles.summary}>{review.summary}</p>
            <div className={styles.managerScores}>
