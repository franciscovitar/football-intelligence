...
export default async function PlayerPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="page-shell">
    ...
    <Section eyebrow="DIMENSIONS" title="Perfil estadístico V2"><div className="dimension-card-grid">{PLAYER_RANKING_DIMENSIONS.filter((item) => item !== "overall").map((dimension) => <DimensionCard key={dimension} label={PLAYER_DIMENSION_LABELS[dimension]} evidence={season.dimensions[dimension]} />)}</div></Section>

    <Section eyebrow="FORM / WINDOWS" title="Cómo cambia la evidencia">{data.windows.length > 1 ? <div className="window-grid">{data.windows.map((item) => <article className="window-card" key={item.window}><span>{WINDOW_LABELS[item.window]}</span><strong>{item.overallScore === null ? "—" : Math.round(item.overallScore)}</strong><EvidenceBadge state={item.evidenceState} /><small>{item.minutes} min · conf. {Math.round(item.confidence * 100)}%</small></article>)}</div> : <EmptyState title="Sin tendencia publicable" missing="Solo existe una ventana de evidencia; no hay dos períodos comparables." unlock="Las ventanas last 3/5/10 se mostrarán cuando tengan muestra real suficiente." compact />}</Section>

    <Section eyebrow="STATISTICS" title="Métricas que sostienen la lectura">{metrics.length ? <div className="metric-category-grid">{Array.from(grouped.entries()).map(([category, items]) => <div className="metric-category" key={category}><h3>{category}</h3>{items.map((metric) => <MetricRow key={`${metric.metricGranularity}-${metric.metricName}`} metric={metric} />)}</div>)}</div> : <EmptyState title="Métricas fuente no disponibles" missing="El perfil existe, pero no hay features reales vinculadas a esta ventana." unlock="La fuente debe aportar valores observados; un campo ausente nunca se muestra como 0." compact />}</Section>

    <Section eyebrow="DIAGNOSTICS" title="Hallazgos deterministas">{diagnostics.length ? <div className="signal-list">{diagnostics.map((item) => <article key={`${item.diagnosticCode}-${item.windowKey}`}><strong>{humanMetric(item.diagnosticCode)}</strong><span>{item.windowKey} · conf. {Math.round(item.confidence * 100)}%</span><small>{Object.entries(item.supportingMetrics).slice(0, 4).map(([key, value]) => `${humanMetric(key)}: ${String(value)}`).join(" · ")}</small></article>)}</div> : <EmptyState title="Sin diagnóstico publicable" missing="Ninguna regla determinista produjo un hallazgo con evidencia suficiente." unlock="Se habilita cuando una divergencia supera los umbrales documentados." compact />}</Section>
