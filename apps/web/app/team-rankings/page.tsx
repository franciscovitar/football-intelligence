...
import { EmptyState, TeamProductCard } from "@/features/product/product-ui";
import { TEAM_DIMENSION_LABELS, WINDOW_LABELS } from "@/lib/product-display";
import { TEAM_RANKING_DIMENSIONS, getProductTeamRankings, type TeamRankingDimension } from "@/lib/queries/product-intelligence";
...
export default async function TeamRankingsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="page-shell"><section className="page-heading"><div><p className="eyebrow">TEAM EXPLORER · V2</p><h1>{TEAM_DIMENSION_LABELS[dimension]}</h1><p>Rankings de equipo con evidencia lista en la dimensión elegida. Una dimensión ausente nunca se interpreta como cero.</p></div></section>
    ...
    {teams.length ? <section className="panel"><div className="ranking-summary"><span>{teams.length} equipos elegibles</span><span>{WINDOW_LABELS[window]}</span></div>{teams.map((team, index) => <TeamProductCard key={team.teamId} team={team} rank={index + 1} />)}</section> : <EmptyState title="Ranking de equipos no disponible" missing={result.status === "ready" ? "Ningún equipo real V2 tiene evidencia lista para esta dimensión y filtros." : result.message} unlock="Se habilita cuando la dimensión alcanza muestra, cobertura y confianza publicables." />}
  </main>;
}
